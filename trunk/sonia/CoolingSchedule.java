 ...
 * "simulated anealing" processes layout, methods for providing acesses to the processes,
 * and the UI for the user, and feedback from the layout process. It also provides
 * the window for displaying all this.  Among other things cooling schedule is used
 * to determine the maximum number of iterations permited by algorithms like KK and FR.
 * This is specified by the position of the last (right most) controlpoint on the plot.
 * This is initially positioned at the end of the plot, so the value is determined
 * by the entry in the "Max Passes" field.  Changing the value in this field and
 * hitting enter will cause the plot to be rescaled.  The control points can be
 * dragged on the plot to specify the shape of the cooling function, and will
    ...
    MouseMotionListener,ActionListener,WindowListener
{
 ...
 // private JLabel MaxPassLabel;
   ...
   * @param numPoints the number of control points, should >= 2 (start and end)
   */
...
/*
  ...
  */
  ...
  /**
   * Calculates and returns the "simulated aneeling" control value apropriate for
   * the pass value.  Essentialy evaluates the segmented function, defined by
   * the control points, for the pass value.  Draws the pass value on the gui
   * @param  pass value at which to evaluate the control function.
   * @return  tempFactor value between 0 and 1 indicating the fraction of the inital value.
   */
  public double getTempFactor(int pass)
  {
    ...
    //figue out the index for the segment of the pice-wise function pass is in
 ...
 * The first elemt [pointIndex][coordinate] is the index for the point. So [2][0]
 * gives the pass value of the 2nd point, and [2][1] gives the height. (IS THIS RIGHT?)
 * @return 2D array of doubles containing the [pointIndex][x,y] control values
 */
   ...
   * have them painted on the screen.  The
   * string should be in the format "(pass,fraction) (pass,fraction) ...".
   * Example: "(0,1.0) (17,0.21) (30,0.12) (100,0.0)" Calls
   * parseValuePair on each pair seperated by whitespace.
   * @param values the string of control values to be tokenized and parsed.
   */
   ...
   * is expected to be in the form "(pointIndex,fractionValue)" with the values
   * seperated by a comma.  Example: "(17,0.21)". All though doubles are returned,
   * the first is parsed to be an int.  WILL IT THROW EXCEPTION ON PARSE ERROR?
   * @param pair the string to be parsed into control values
  ...
  private double[] parseValuePair(String pair)
  {
    ...
    if (commaIndex > 0)
    {
      ...
      value[1] = Double.parseDouble(pair.substring(commaIndex+1,pair.length()-1));
    }
    else
    {
      ///should spit to control, but cooling schedule doesn't have and instance...
   ...
   * or logging, and reparseing.  String will be in the format "(pass,fraction) (pass,fraction) ...".
   * Example: "(0,1.0) (17,0.21) (30,0.12) (100,0.0)"
  ...
  /**
   * Draws a horisontal line on the plot at a height corresponding to the passed
   * double value when scaled to the plot.
   * @param value the data value to draw the line for.
   */
  ...
  /**
   * Plots a point to show the convergance of the algorithm, the difference between
   * the value at the previous time and the ccurrent value.
   * @param pass the the current pass number
   * @param value the value at the current pass
   */
   ...
   * @param pass the pass indicating where the line should be drawn
   */
  ...
  /**
    *  Converts the stored pass,fractionOfStart control value pairs to screen x,y pairs
    * for plotting and stores the results in the plots cash for redraws.
    */
  ...
  /**
   * Converts the current ploted screen points of the control points to the internal
   * pass, fractionOfStart control points and stores them.  Used to read values
   * when adjusted by user in the gui.
   */
...
//NEED TO SET TO FEWER SIGNIFICANT DIGITS
  ...
  /**
   * Calls TextPropmter with a string of control values from getCtlValString() to
   * let the user modify or record them, parses the result and sents the points
   * accordingly.
   */
  public void setValues()
  {
    TextPrompter prompter = new TextPrompter(this, getCtlValString(),
...
//  }
  ...
  private int getPointAt(int x, int y)
  {
    ...
    for (int i=0; i<bendPoints; i++)
    {
      if (((x>pointsX[i]-5)&(x<pointsX[i]+5))&(((y>pointsY[i]-5)&(y<pointsY[i]+5))))
      {
        pointNum = i;
      }
    }
   ...
   * control point around on the screen, adjusting the control values in real time.
   * @param e the mouse event to query for coordinates.
   */
...
//the rest have to be there to satisfy WindowListener
  ...
  //accessors----------
  /**
   * Returns the user-specified maximum number of passes for the layout algorithm,
   * as defined by the the horisonal position of the last control point.
   ...
   * retuned can be less than this value. This essentialy sents the horrisontal
   * scale for the cooling schedule.
   * @param passes the maximum number of passes
   */
   ...
   * @param value the double value corresponding to the maximum value of the plot.
   */
