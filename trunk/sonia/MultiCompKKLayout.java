 ...
 * (Kamada, K., Kawai, S. (1989) <CITE>An Algorithm for Drawing General Undirected Graphs.</CITE>
 * Information Processing Letters 31, 7-15) this version is modified in several ways from the original.  It
 * give the users much more explicit control over stopping times and energy
 * minimization.  It is also configured to break networks of multiple components
 * into individiual networks and run the algorithm independently. In additon to
 * stopping when the specified minimum energy is reached, the algorithm also
 * stops when a set number of passes (determined by the schedule) have been
 * completed, or when the layout cesease to converge.
 * (more about kk)
 *<BR>
 * The main loop of the algorithm runs on its own "layout runner" thread so that
 * it can update the display, pause, etc.  The two "regular" KK loops are embedded
 * in a third "epsilon minimizing" loop which gradually increases the minimuim
 * energy target the KK loop is trying to achive.  The advantage of doing this
 * incrementally is that if the loop stops short, either because the desired
 * optimization is impossible in two dimensions, or by exceeding the maximum
 * number of iterations, at least some optimization will have been performed.
 * <BR>
 * <BR>
 * What makes this slightly confusing is that, unlike the FR layout, the cooling
 * schedule does not directly linked to the number of primary passes through
 * the algorithm.  Instead, it controlls the fraction by which the target
 * epsilon is reduced each time the main KK loop reaches its goal.  The initial
 * value is that of the worst-positioned node when the layout begins.  Obviously,
 * this could use some conceptual improvement.
 * <BR><BR>
 * LAYOUT PARAMETERS:
 * </P><P>
 ...
 * </P><P>
 * "min epsilon" - sets the final target energy value for energy minimization.  Smaller
 * values will give better layouts, but you may need to increase the number passes,
 * and it will take longer.
 * </P><P>
 * "springConst" - not sure what this does.  How "springy" the springs are?
 * </P><P>
 * "cool factor" - how much the target energy will be reduced in each pass through
 * the "epsilon minimizing" loop.
 * </P>
 *
 ...
 * network as a virtual collection of weights (nodes) connected by springs (arcs)
 * with a degree of elasticity and a desired resting length.  The problem then is
 * to reposition the nodes until all the springs are as relaxed as possible.
 * Kamada and Kawai's version utilized simple force equations and a Newton-Raphson
 * steepest decent approach  to locating the optimal configuration.  In addition,
 * they perform an All-Pairs-Shortest-Path distance calculation to determine
 * the desired spring lengths between nodes.  Interestingly, because the KK
 * algorithm is working to optimize distances between every pair of nodes,
 * this makes it conceptually similar to a non-metric multidimensional scaling
 *  approach.  Because we have adapted the algorithm to work with weighted
 * di-graphs and the possibility of multiple components, some transformations
 * (transformation to dissimilarity, symetrization) are usually necessary.
 *  When there are disconnected components in the graph, the algorithm is run
 * independently on each.
 * </P><P>
 * The original KK algorithm works as follows: First, the algorithm calculates the "energy" (sum of spring tension) for
 * each node.  It then loops over all the nodes to find the one with the highest
 * energy (worst positioning), and begins iterating the Newton-Raphson stage to
 * compute new positions for the node until its energy is below epsilon
 * (the target parameter).  At this point, it again looks for the node with the
 *  highest energy and begins moving it.  This process continues until there is
 * no node with energy above epsilon, and then the algorithm is done.
 * </P><P>
 * This procedure needs several extensions to work for our purposes.  To get an
 * optimal layout, the epsilon parameter must be set to the minimum
 * (lowest stress) possible for the graph, but there is no simple way of
 * determining this before running the algorithm.  If the value is set too low
 * for a particular graph, the algorithm will not stop unless a maximum number
 *  of iterations is exceeded.  But this gives no guarantee that the user will
 ...
 * exceed the "max passes" parameter, the layout will "bonk" and record an error)
 * In addition, the process of
 * deterministically moving the "worst" node to the best position and then
 * re-evaluating it can be a problematic means of exploring the search space.
 *  It is generally more effective to gradually optimize the entire graph.
 * </P><P>
 ...
 * parameter, the target value is improved (reduced by a fraction, the
 * "cooling factor" parameter) and the algorithm continues until the layout
 * is better than a global "minimum epsilon".  This ensures that if the layout
 * exceeds the maximum number of iterations or lands in a loop, at least a basic
 * level of optimization will have occurred.  Informal testing seems to show this
 ...
 * minimum value.  This makes it possible to assess whether the parameters
 * should be changed so as to allow additional iterations in difficult layouts.
 * This suggests the possibility of building a "smarter" algorithm which stops
 * when additional iterations no longer give any improvement, but this will
 * require additional testing and parameter tuning.
 * </P><P>
 * KK was originally intended for undirected, non-weighted, fully connected graphs.
 *  We took a simplistic extension to weighted graphs, symmetrizing the input
 * digraph and using an All-Pairs-Shortest-Path algorithm to give a matrix of
 *  graph distances which are multiplied by an "optimal distance" parameter to
 ...
 * with more than one node.  This means that there is nothing explicitly separating
 * the components, and they sometimes overlap on screen.  Several people have
 * reported some success using a "phantom network" to minimally connect the graph
 *  in order to avoid the component positioning problem.  This approach sounds
 * promising but needs some additional thought and testing.
 * </P><P>
 ...
 *
 * It is in fact the algorithm we have had the most success with, perhaps because
 * it is simply the algorithm we have devoted the most time too, or because it
 * is better suited to the concepts of social space we are using. There are
 * several strong advantages to the KK algorithm. One is that the Newton-Raphson
 * optimization procedure is completely deterministic - given parameters and
 * starting positions, it will arrive at the same solution on the same graph.
 *  This is convenient when working with a sequence of time-based networks.
 * If we start the layout from the previous slice's node coordinates and there
 * is no change in the arc weights, then there should be no change in node
 *  positions.  In addition, if there are small changes in weights between the
 * layouts, or a small number of arcs added or deleted, then KK will likely find
 * new minimum that is close in coordinate space to the previous solution.
...
public class MultiCompKKLayout implements NetLayout, Runnable
{
  ...
  /**
   * Instantiates the layout with reference to the main SoniaController
   * and the the SoniaLayoutEngine connected to this layout
   * @param cont the main controller
   * @param eng the layoutEngine
   */
   ...
   * "optimum distance" - the desired distance in pixels between nodes of distance 1<BR>
   * "min epsilon" - the stopping "energy" of the layout, how far it will try to optimise<BR>
   * "springConst" - the spring constant from KK, roughly how hard nodes try to jump to positions
   * @param settings the dialog the parameters will be added to
   */
   ...
   * @param s the slice the layout will be performed on
   * @param w width of the layout area (in pixels)
   * @param h height of the layout are (in pixels)
   * @param set settings dialog containing the values of the settings.
   */
  public void applyLayoutTo(LayoutSlice s, int w, int h, ApplySettingsDialog set)
  {
    slice = s;
    settings = set;
  ...
  /**
   * Runs the KK layout algorithm.  First it symetrizes the network to max values
   * and then breaks it into components.  Then it converts each of the networks
   * into Subnets (to perserve the relations between old and new indexes, ids, etc)
   * and runs KKLoop() on all the subnets with more than 1 node.  Asks the engine
   * to finishLayout() when done.  Should only be called internaly when the thread
   * starts.
   */
  public void run()
  {
  ...
  /**
   * Gets the similarity matrix from the subnet, and converts it to disimilarity
   * using the NetUtils.getReverse() method with the min and max values from the
   * engine.  Converts the result into a distance matrix with an All Pairs Shortest
   * Path algorithm (Dijkstra's) NetUtils.getAllShortPathMatrix().  Gets the
   * spring constant and uses calcKMatrix() to setup a matrix of forces. Gets the
   * "optimum distance" parameters: the desired pixels distance between nodes.
   * Gets the starting coordinates form the subnet.  Calculates the "initial
   * energy" of the network (subnet)
   * <BR>
   * <BR>
   * The optimization loop proceeds as follows:
   * First, the node with the highest energy (worst position) is located.
   */
  private void KKLoop(Subnet subnet)
  {
   ...
   DenseDoubleMatrix2D distMatrix =
      ...
      //sets up kmatrix of forces (optimal [but not always achieveable] energies?)
      ...
      while ((maxDeltaM > epsilon) & ((previousMaxDeltaM - maxDeltaM) > 0.1)
          & noBreak)
        {
              ...
              & (passes % settings.getRepaintN() == 0))
          {
            ...
            {
              utils.centerLayout(slice, (int)width, (int)height, sliceXCoords,
                                 sliceYCoords,settings.isIsolateExclude());
            }
  ...
  */
  ...
  /**
   * Returns the layout info string for the log, which may contain information about
   * starting and stopping values, etc.
   */
