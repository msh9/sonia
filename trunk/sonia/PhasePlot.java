...
/**
   * Gives a "phase-space" graphical representation of the network data and the settings
   * selected in the layoutSettings dialog, indicates the current slice and
   * render window, allows navigation by clicking on slices. Indicates the
   ...
   * show the slice widths (shown in white) and network events.  ArcEvents are shown in blue,
   * NodeEvents are shown in cyan.  Their position indicates the start time, with
   *  the length of the horizontal bar indicating the duration of the event.
   * The x position in normaly determined randomly (so the point clouds
   *  create the effect of a pseudo-histogram) but they can be sorted in
   * chronological order by check the "Sort Events" option.
   * <BR><BR>
   * When a LayoutWindow is displayed, the PhasePlot also serves as a navigation
   * tool.  Clicking on the slice will ask the layout engine to change to the slice.
   * The current slice is outlined in red, and the region of time being displayed
   * by the current RenderSlice is shown in orange. Keeping the plot displayed
    ...
    MouseMotionListener,MouseListener
{
                   ...
                   LayoutSettingsDialog set)
  {
    ...
    events = new ObjectArrayList(data.getEventsFromTo(plotStart,plotEnd).toArray());
    ...
    c.gridx=0;c.gridy=0;c.gridwidth=3;c.gridheight=1;c.weightx=1;c.weighty=1;
    this.add(drawArea,c);
    c.anchor = c.SOUTH;
    c.gridx=2;c.gridy=1;c.gridwidth=1;c.gridheight=1;c.weightx=1;c.weighty=1;
    this.add(sortBox,c);
  ...
  public void plotData(Graphics2D graph)
  {
    int topPad = this.getInsets().top+sidePad;
   ...
   for (int s = 0; s<numSlices; s++)
   {
     ...
     sliceRect.setRect(sliceStart*scaleFactor+sidePad,
       (double)topPad,sliceDuration*scaleFactor,(double)plotHeight);
     graph.fill(sliceRect);
     //draw the starting line for the slice
     graph.setColor(Color.green);
     startLine.setLine(sliceStart*scaleFactor+sidePad,
       (double)topPad,sliceStart*scaleFactor+sidePad,(double)xAxis);
     graph.draw(startLine);
     //draw the ending line for the slice
     graph.setColor(Color.red);
     endLine.setLine((sliceStart+sliceDuration)*scaleFactor+sidePad,
       (double)topPad,(sliceStart+sliceDuration)*scaleFactor+sidePad,(double)xAxis);
     graph.draw(endLine);
    ...
    for (int i = 0; i < events.size(); i++)
    {
      ...
      double from = (event.getObsTime()-dataOffset)*scaleFactor+sidePad;
      double to = (event.getEndTime()-dataOffset)*scaleFactor+sidePad;
      ...
      //move to new postion
      phaseBar.setLine(from,(xAxis - yStep*i),to,(xAxis - yStep*i));
    ...
    if ((renderStart >= 0) & (renderEnd >= 0))
    {
      ...
      sliceRect.setRect((renderStart-dataOffset)*scaleFactor+sidePad,
                        (double)topPad-3.0,(renderEnd-renderStart)*scaleFactor,
    ...
    sliceRect.setRect((currentSliceIndex*sliceDelta)*scaleFactor+sidePad,
       (double)topPad-1,sliceDuration*scaleFactor,(double)plotHeight+2);
    graph.draw(sliceRect);
...
//and draw the scale on to check things
   ...
   // graph.setColor(Color.black);
   // Line2D.Double mouseBar = new Line2D.Double();
    //mouseBar.setLine((double)mouseX,topPad,(double)mouseX,xAxis+3);
   // graph.draw(mouseBar);

  }

  /**
   * records the start and end time so the render slice can be drawn
   * on the plot
   */
  ...
  public void mouseMoved(MouseEvent event)
  {
    ...
    double mouseTime = ((double)mouseX-sidePad)/scaleFactor+dataOffset;
