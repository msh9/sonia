...
public class SoniaLayoutEngine
{
  ...
  private LayoutWindow display; //eventually need to allow for multiples of these
  private ArrayList layoutSlices; //slices, bins of all events to be in same layout
 // private Vector renderSlices;
  ...
  /**
   * Class responsible for maintaining data structures for layouts, coordinating
   * layout algorithms, suppling graphics to consumers, etc
   * @param controller the main sonia controller
   * @param data the network data to divide into slices
   * @param name  the name of the engine (to show in the window)
   */
  public SoniaLayoutEngine(SoniaController controller, NetDataStructure data,
                           String name)
  {
    control = controller;
    netData = data;
    ...
    //build appropriate layout window
    display = new LayoutWindow(control,this,dispInitWidth,dispInitHeight);
     //probably should ask for kind of layout here
    windowSettings = new LayoutSettingsDialog(control,this,display);
    //tell the settings dialog what the start and end times for the data are
    ...
    //timePlot = new PhasePlot(this,data,windowSettings);
  ...
  public void showApplyLayoutSettings()
  {
    if (settings == null)
    {
      settings = new ApplySettingsDialog(control,this,display,currentLayout);
    }
    settings.showDialog();
  ...
  /**
   * Creates and displays  a PhasePlot showing the location of each arc and node event in time,
   *  and how the slices land.
   */
...
/**
 *Sets up the layout and data sctructures, asking netData to generate a vector
 * of slices from the given range, finding max values (to be used in converting
 * similarity to disimilarity matrix), shows slice, etc.
 * @param sliceStart start time for the range of slices
 * @param sliceEnd end time for the range of slices
 * @param sliceDuration how long ("thick") the slices are
 * @param sliceDelta the offset between the start of succesive slices
 * @param aggregateType the method used to aggregate ties within a slice
 * @param layout the layout algorithm to position the nodes
 * @param interp the coordinate interpolator to be used for animating transitions
 */
  public void setupLayout(double sliceStart, double sliceEnd,
                          double sliceDuration, double sliceDelta,
                            int aggregateType,NetLayout layout,
                          CoordInterpolator interp)
  {
   ...
   * number.
   * @param num the slice number to change to
   */
...
/** Applies the layout to the current slice.  Called by the "Re-apply" button.
  * Uses the current layout settings, or calles the ApplyLayoutSettings dialog
  ...
  */
  ...
  /**
   * Starts an independent thread to run the layout algorithms, this allows screen
   * redraws and events (like pause) to take place.  Thread calls
  ...
  /**Applies a layout algorithm to the passed slice.  First checks that no other
   * layout is working on the slice, then locks it, gets the current screen
   * dimensions of the layout, sets the starting coordinates of the slice
   * to the appotion specified in the settings dialog, asks the layout algorithm
   * to start, and records the starting info in the log.
   * @param slice the slice which the layout will be applied to
   */
  public void applyLayoutTo(LayoutSlice slice)
  {
    ...
    else
    {
      ...
      //PROBABLY LAYOUT SHOULD WORK ON AN COORD SYSTEM INDEPENT OF DISPLAY
      ...
      else if ( settings.getStartCoordCode() == settings.PREV_SLICE)
      {
        ...
        if (thisIndex > 0)
        {
          LayoutSlice prevSlice = (LayoutSlice)layoutSlices.get(thisIndex-1);
      ...
      else if (settings.getStartCoordCode() == settings.ORIG_FILE)
      {
        ...
        LayoutSlice copySlice =
            netData.makeLayoutSlice(slice.getSliceStart(),slice.getSliceEnd(),0);
      ...
      control.showStatus("Starting layout on slice "+layoutSlices.indexOf(slice)+
                         "\nstart time:"+slice.getSliceStart()+
                  "\nend time:"+slice.getSliceEnd());

    }
  }

  /**
   * Does any final repositioning (rescaling, recentering, Isolate repositioning,
   * etc) of the nodes,  logs layout settings and results, and sets the slice
   * to "finished" status, updates the display.
   * @param layout the layout to finish
   * @param slice the slice being worked on
   * @param width the width (in pixels) of the layout area
   * @param height the height (in pixels) of the layout area
   */
                           ...
                           double height)
  {
    ...
    {
      LayoutUtils.centerLayout(slice, (int)width,(int)height,settings.isIsolateExclude());
    }
    else if (settings.isEndBarycenter())
    {
      LayoutUtils.barycenterLayout(slice, (int)width,(int)height,settings.isIsolateExclude());
    }
    ...
    control.showStatus("Slice "+layoutSlices.indexOf(slice)+" Layout finished");
   ...
   * comparing screen distances to desired matrix distances)
   *  calculations for current slice, displays them as
   * a shepard's plot in the stress window.  If there is no stress window, it
   * makes a new StressInfo object/window.
   */
  public void calcStress()
  {
    //double optDist = settings.get; //temporary test!!
    //double stress = NetUtils.getStress((LayoutSlice)layoutSlices.get(currentSlice));
   ...
   * @param start the start of the RenderSlice's range
  ...
  */
   ...
   * if they have not been set yet, it returns the xcoords stored in the slice.
   */
  public double[] getCurrentXCoords()
  {
    ...
    {
      currentXcoords = ((LayoutSlice)layoutSlices.get(currentSlice)).getXCoords();
    }
   ...
   * if they have not been set yet, it returns the ycoords stored in the slice.
   */
  public double[] getCurrentYCoords()
  {
    ...
    {
      currentYcoords = ((LayoutSlice)layoutSlices.get(currentSlice)).getYCoords();
    }
   ...
   * (within the range of those slices) calculates new intermediate coordinates
   * according to the current interpolation scheme, and stores them as the
   * current x and y coords to be used when drawing the network to the screen.
   * @param startSlice the slice interpolation will start from
   * @param endSlice the slice interpolation will end at
   * @param time the time for which to calculate the interpolation
   */
  public void interpCoords(LayoutSlice startSlice, LayoutSlice endSlice, double time)
  {
    //WHAT HAPPENS WHEN SLICES OVERLAPP? Should we Average all the slices within
    //that time?
  ...
  /**
   * Sets the current x and y screen coordinates for the nodes to the coordinates
   * stored in the slice corresponding to the passed slice number.
   * @param sliceNum the number of the slice to get coords from
   */
  public void setCoordsToSlice(int sliceNum)
  {
    ...
    {
      currentXcoords = ((LayoutSlice)layoutSlices.get(sliceNum)).getXCoords();
      currentYcoords = ((LayoutSlice)layoutSlices.get(sliceNum)).getYCoords();
    }
  }

  /**
   * Returns a Vector of matricies corresponding to each of the slices stored in
   * the engine.  Calls NetUtilities.getMatrix() on each slice. Called when
  ...
  /**
   * Asks the display to start making a movie with the passed SoniaMovieMaker by
   * callinging makeMovie(exporter) on the LayoutWindow
   * @param exporter the SoniaMovieMaker to save the movie frames to
   */
   ...
   * doesn't exist yet, it returns the default initial values. result is in pixels
   */
  ...
  /** Checks if the passed value is > 0, sets the with of the display (layoutWindow)
   *  to the value, sets the height to its current value.  Asks the window to redo
   * layout to position components.
   * @param width the number of pixels wide to set the layout area
   */
  ...
  /**
     * Asks the display for the height of the frame using getHeight().  if display
     * doesn't exist yet, it returns the default initial values. result is in pixels
   */
  ...
  /** Checks if the passed value is > 0, sets the width of the display (layoutWindow)
   *  to the value (adding the bottomPad to make room for the buttons), sets the
   * height to its current value.  Asks the window to redo layout to position
   * components.
   * @param height the number of pixels high to set the layout area
   */
  ...
  /**
     ...
     * The height is smaller than the window height by 2*pad (to make sure that
     * the nodes don't quite touch the edge of the screen) and also has the
     * height of the bottom pad subtracted (to leave room for the buttons).
   */
  ...
  {
    return currentLayout.getLayoutType()+" "+currentLayout.getLayoutInfo();
  ...
  /** Sets how often the layouts will be drawn to the screen while the algorithm
   * is optimizing.  If n > 0, showUpdates will be set to true.  Layouts will be
   * drawn on every "n"th optimization loop, where n is the past int.  Default is 0.
   * @param n draw layout to screen every n loops
   */
  public void setUpdatesN(int n)
  {
    updatesN = n;
    if (n > 0) showUpdates = true;
  }
  ...
  /**
   * returns an array of booleans corresponding to each slice, true if
   * there are errors.  WARNING, elements may be null if layout has not
   * been applied!
   */
  ...
  /**
   * returns the stored maximum value of a cell in the network matricies, calc'd
   * by setupLayout().  Used for similarity/disimilarty conversions
   */
  ...
  /**
   * returns the stored minmum value of a cell in the network matricies, calc'd
   * by setupLayout().  Used for similarity/disimilarty conversions
   */
  ...
  /**
   * returns the LayoutSlice with the corresponding index.  Does NOT check range
   * @param sliceNum index of slice to be returned
   */
  public LayoutSlice getSlice(int sliceNum)
 {
  ...
  /**
   * Sets the number intermediary interpolation ("tweening") frames to be drawn
   * while animating transitions between slices.
   * @param frames  how many frames to draw during transition
   */
  ...
  /**
   * Returns the current number of intermediary interpolation ("tweening") frames to be drawn
   * while animating transitions between slices.
   */
  ...
  /**
   * Gives the engine a chance to do any clean up (like asking layouts to dispose
   * of schedules..
   */
